...
class ParadoxCompiler(compiler.SQLCompiler):
    ...
    def visit_fromclause(self, fromclause, **kwargs):
        ...
        pprint(kwargs)
        print("-------\n\n")
        super(ParadoxCompiler, self).visit_fromclause(self, fromclause, **kwargs)
    ...
    def get_column_specification(self, column, **kw):
        if column.table is None:
            raise exc.CompileError(
                "Paradox requires Table-bound columns in order to generate DDL"
            )
...
class ParadoxDialect(default.DefaultDialect):
    ...
    statement_compiler = ParadoxCompiler
